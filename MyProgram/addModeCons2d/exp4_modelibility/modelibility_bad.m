...
EXP_improvement=zeros(1,numberOfExp);
err_BAR=zeros(59,numberOfExp);
for e=1:numberOfExp
close all;
N = 300;   %粒子总数
Q = 5;      %过程噪声
...
M_Pro_1=zeros(MNumber,T/stageNumber-1);
M_Pro_2=zeros(MNumber,T/stageNumber);
M_Pro_3=zeros(MNumber,T/stageNumber);
M_Pro_4=zeros(MNumber,T/stageNumber);
M_Pro_5=zeros(MNumber,T/stageNumber);
M_Pro_6=zeros(MNumber,T/stageNumber-1);
...
for k = 2 : T
    ...
    %模拟一个弧线运动的状态
        ...
        randMode = M(m,:);
        X(1, k) = X(1, k) + distance * (randMode(1)*1.5+randMode(2)*(-1.5)) + wgn(1, 1, 10*log10(Q));
        X(2, k) = X(2, k) + distance * (randMode(3)*1.5+randMode(4)*(-1.5)) + wgn(1, 1, 10*log10(Q));
...
%     end
    ...
    figure(2)
    set(gca,'FontSize',12);
    set(gcf,'position',[50 400 450 400]);
    ...
    %pause(0.1);
    figure(3);
    set(gcf,'position',[515 400 450 400]);
    %hold on;
...
%
% fprintf('The accuracy of mode recognition in each frame is:%.4f\n',modeCorrect/T);
...
% fprintf('The improvement degree is:%.4f\n',recACC-(modeCorrect/T));
meanDelay=sum(timeDelay)/stageNumber;
% fprintf('The mean time delay of reconition is:%.4f\n',meanDelay);

% 
M_Pro_1=M_Probability(:,2:10);
M_Pro_2=M_Probability(:,11:20);
M_Pro_3=M_Probability(:,21:30);
M_Pro_4=M_Probability(:,31:40);
M_Pro_5=M_Probability(:,41:50);
M_Pro_6=M_Probability(:,51:59);
%
mean_Pro_1=mean(M_Pro_1,2);
mean_Pro_2=mean(M_Pro_2,2);
mean_Pro_3=mean(M_Pro_3,2);
mean_Pro_4=mean(M_Pro_4,2);
mean_Pro_5=mean(M_Pro_5,2);
mean_Pro_6=mean(M_Pro_6,2);


EXP_frameacc(e)=modeCorrect/T;
EXP_acc(e)=recACC;
EXP_improvement(e)=EXP_acc(e)-EXP_frameacc(e);
EXP_time(e)=meanDelay;
EXP_framedelay(e)=timeOfAll/T;
EXP_recdelay(e)=sum(timeDelay)/stageNumber;

err_BAR(:,e)=err(2:60);
% fprintf('The mean error of tracking is:%.4f\n',mean(err_BAR(:,e)));
...
% meanEXP_improvement=mean(EXP_improvement);
% meanEXP_meanDelay=mean(EXP_time);
% meanEXP_meanError=mean(err_BAR(:));
...
% fprintf('The mean improvement degree in 100 times experiment is:%.4f\n',meanEXP_improvement);
% fprintf('The mean recognition delay in 100 times experiment is:%.4f\n',meanEXP_meanDelay);
% fprintf('The mean tracking error in 100 times experiment is:%.4f\n',meanEXP_meanError);
