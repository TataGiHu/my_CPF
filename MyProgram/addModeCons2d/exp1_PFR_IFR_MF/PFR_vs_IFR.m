...
numberOfExp=100;
PFR_err_BAR=zeros(59,numberOfExp);
IFR_err_BAR=zeros(59,numberOfExp);

IFR_EXP_frameacc=zeros(1,numberOfExp);
IFR_EXP_acc=zeros(1,numberOfExp);
% EXP_framedelay=zeros(1,numberOfExp);
% EXP_recdelay=zeros(1,numberOfExp);
IFR_EXP_time=zeros(1,numberOfExp);
for e=1:numberOfExp
close all;
N = 300;   %粒子总数
Q = 5;      %过程噪声
...
fprintf('The average output time is:%.4f\n',outTime/T);
% 
% 
% 
IFR_EXP_frameacc(e)=modeCorrect/T;
IFR_EXP_acc(e)=recACC;
IFR_EXP_time(e)=outTime/T;
% EXP_framedelay(e)=timeOfAll/T;
% EXP_recdelay(e)=sum(timeDelay)/stageNumber;
IFR_err_BAR(:,e)=err(2:60);
...
IFR_meanEXP_frameacc=mean(IFR_EXP_frameacc);
IFR_meanEXP_acc=mean(IFR_EXP_acc);
IFR_meanEXP_time=mean(IFR_EXP_time);

% fprintf('The mean frame accuracy in 100 times experiment is:%.4f\n',IFR_meanEXP_frameacc);
% fprintf('The mean accuracy of recognition in 100 times experiment is:%.4f\n',IFR_meanEXP_acc);
% fprintf('The mean output time after start in 100 times experiment is:%.4f\n',IFR_meanEXP_outputTimeAfterSTART);



%%

PFR_EXP_frameacc=zeros(1,numberOfExp);
PFR_EXP_acc=zeros(1,numberOfExp);
% EXP_framedelay=zeros(1,numberOfExp);
% EXP_recdelay=zeros(1,numberOfExp);
PFR_EXP_time=zeros(1,numberOfExp);
PFR_EXP_afterTime=zeros(1,numberOfExp);
for e=1:numberOfExp
close all;
N = 300;   %粒子总数
Q = 5;      %过程噪声
...
fprintf('The accuracy of recognition is:%.4f\n',recACC);
fprintf('The average output time is:%.4f\n',outTime/T);
fprintf('The average time after frame is:%.4f\n',afterTime/T);
% 
% 
% 
PFR_EXP_frameacc(e)=modeCorrect/T;
PFR_EXP_acc(e)=recACC;
PFR_EXP_time(e)= outTime/T;
PFR_EXP_afterTime(e)= afterTime/T;
% EXP_framedelay(e)=timeOfAll/T;
% EXP_recdelay(e)=sum(timeDelay)/stageNumber;
PFR_err_BAR(:,e)=err(2:60);
...
PFR_meanEXP_frameacc=mean(PFR_EXP_frameacc);
PFR_meanEXP_acc=mean(PFR_EXP_acc);
PFR_meanEXP_time=mean(PFR_EXP_time);
PFR_meanEXP_afterTime=mean(PFR_EXP_afterTime);

figure(6);
plot(1:numberOfExp,PFR_EXP_frameacc,'-.bo',1:numberOfExp,IFR_EXP_frameacc,'-.r+');
xlabel('Experiment');ylabel('Mean frame accuracy');
title('Mean Frame accuracy in 100 Experiments');
legend('PFR method','IFR method');

figure(7);
plot(1:numberOfExp,PFR_EXP_acc,'-bo',1:numberOfExp,IFR_EXP_acc,'-r+');
xlabel('Experiment');ylabel('Mean frame accuracy');
title('Mean accuracy of Recognition in 100 Experiments');
legend('PFR method','IFR method');

figure(8);
plot(1:numberOfExp,PFR_EXP_time,':bo',1:numberOfExp,IFR_EXP_time,':r+');
xlabel('Experiment');ylabel('Mean frame accuracy');
title('Mean time delay in 100 Experiments');
legend('PFR method','IFR method');

fprintf('The mean frame accuracy in 100 times experiment with PFR:%.4f\n',PFR_meanEXP_frameacc);
fprintf('The mean accuracy of recognition in 100 times experiment with PFR:%.4f\n',PFR_meanEXP_acc);
fprintf('The mean output time 100 times experiment with PFR:%.4f\n',PFR_meanEXP_time);
% fprintf('The mean time after frame 100 times experiment with PFR:%.4f\n',PFR_meanEXP_afterTime);

fprintf('The mean frame accuracy in 100 times experiment with IFR:%.4f\n',IFR_meanEXP_frameacc);
fprintf('The mean accuracy of recognition in 100 times experiment with IFR:%.4f\n',IFR_meanEXP_acc);
fprintf('The mean output time in 100 times experiment with IFR:%.4f\n',IFR_meanEXP_time);



figure(9);
boxplot(PFR_err_BAR);
title('Error bar with PFR');
xlabel('Experiment');
ylabel('Error bar');

figure(10);
boxplot(IFR_err_BAR);
title('Error bar with IFR');
xlabel('Experiment');
ylabel('Error bar');

PFR_meanEXP_frameacc_std=std(PFR_EXP_frameacc);
PFR_meanEXP_acc_std=std(PFR_EXP_acc);
IFR_meanEXP_frameacc_std=std(IFR_EXP_frameacc);
IFR_meanEXP_acc_std=std(IFR_EXP_acc);
fprintf('The mean frame accuracy std of recognition in 100 times experiment with PFR:%.4f\n',PFR_meanEXP_frameacc_std);
fprintf('The mean accuracy std of recognition in 100 times experiment with PFR:%.4f\n',PFR_meanEXP_acc_std);
fprintf('The mean frame accuracy std of recognition in 100 times experiment with IFR:%.4f\n',IFR_meanEXP_frameacc_std);
fprintf('The mean accuracy std of recognition in 100 times experiment with IFR:%.4f\n',IFR_meanEXP_acc_std);
